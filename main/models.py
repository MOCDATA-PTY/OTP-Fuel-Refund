...
class RootPassword(models.Model):
    """
    Model to store the root password for dashboard access.
    Only one instance should exist.
    """
    password_hash = models.CharField(max_length=128, help_text="Hashed root password")
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    is_active = models.BooleanField(default=True)
    
    def set_password(self, raw_password):
        """Set the root password using Django's password hashing."""
    ...
    @classmethod
    def get_or_create_root_password(cls, default_password="admin123"):
        """Get the root password instance or create one with default password."""
        root_pass, created = cls.objects.get_or_create(
            defaults={'is_active': True}
        )
        if created:
            root_pass.set_password(default_password)
            root_pass.save()
        return root_pass
    
    def __str__(self):
        return f"Root Password (Active: {self.is_active})"
